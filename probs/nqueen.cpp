...
void binDfs(int ind, vector<int> &store)
{
    if (ind == 3)
    {
        // print store
...
void tourDfs(int x, int y, vector<vector<int>> &board,
                    int &moves, int row, int col)
{
    ...
    for(int i = 0; i < 8; i++)
    {
        int nextX = x + x_add[i];
        int nextY = y + y_add[i];
        // vheck valid cell(-1) and boundary 
        if(nextX >= 0 && nextX < row &&
           ...
           board[nextX][nextY] == -1)
           {
                ...
                tourDfs(nextX, nextY, board, moves, row, col);
                moves--;
                board[nextX][nextY] = -1;
...
void knightstour(int row, int col)
{
    vector<vector<int>> board(row, vector<int>(col, -1));
    int moves = 1;
    board[0][0] = 1;
    //      x  y,
    tourDfs(0, 0, board, moves, row, col); 
}
